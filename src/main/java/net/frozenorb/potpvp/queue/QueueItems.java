...
public final class QueueItems {
    ...
    static {
        ...
        ItemUtils.setDisplayName(
            JOIN_SOLO_RANKED_QUEUE_ITEM, 
            LEFT_ARROW + 
            GREEN.toString() + BOLD + "Join " + 
            AQUA.toString() + BOLD + "Ranked " + 
            GREEN.toString() + BOLD + "Queue" + 
            RIGHT_ARROW
        );
        ItemUtils.setDisplayName(
        ...
        ItemUtils.setDisplayName(
            JOIN_PARTY_UNRANKED_QUEUE_ITEM, 
            LEFT_ARROW + 
            GREEN.toString() + BOLD + "Join 2v2 Unranked" + 
            RIGHT_ARROW
        );
        ItemUtils.setDisplayName(
            LEAVE_PARTY_UNRANKED_QUEUE_ITEM, 
            LEFT_ARROW + 
            RED.toString() + BOLD + "Leave 2v2 Unranked Queue" + 
            RIGHT_ARROW
        );

        ItemUtils.setDisplayName(
            JOIN_PARTY_RANKED_QUEUE_ITEM, 
            LEFT_ARROW + 
            GREEN.toString() + BOLD + "Join 2v2 Ranked" + 
            RIGHT_ARROW
        );
        ItemUtils.setDisplayName(
            LEAVE_PARTY_RANKED_QUEUE_ITEM, 
            LEFT_ARROW + 
            RED.toString() + BOLD + "Leave 2v2 Ranked Queue" + 
            RIGHT_ARROW
        );
    }
}